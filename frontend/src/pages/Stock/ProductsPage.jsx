        ...
        <div className="w-full md:w-56">
          ...
          >
            ...
            <optgroup label="Other / Subcategories">
              {categories
                .filter((c) => Boolean(c.parentCategory))
                .map((c) => (
                  <option key={c._id} value={c._id}>
                    ↳ {c.name} ({c.parentCategory?.name || 'Subcategory'})
                  </option>
                ))}
            </optgroup>
          </select>
                        ...
                        {p.category ? (
                          p.category.parentCategory ? (
                            <div className="flex flex-col items-start gap-1">
                              <span className="font-semibold text-slate-900 bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-md border border-indigo-100">
                                {p.category.name}
                              </span>
                              <span className="text-[10px] text-slate-500 flex items-center gap-1">
                                <span className="text-slate-400">Parent:</span>
                                <span className="font-medium text-slate-700">
                                  {p.category.parentCategory?.name || 'Parent Category'}
                                </span>
                              </span>
                            </div>
                          ) : (
                            <div className="flex flex-col items-start gap-0.5">
                              <span className="font-semibold text-sky-800 bg-sky-50 px-2.5 py-0.5 rounded-md border border-sky-200">
                                {p.category.name}
                              </span>
                              <span className="text-[10px] font-semibold text-sky-600 uppercase tracking-wider pl-0.5">
                                Parent Category
                              </span>
                            </div>
                          )
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <optgroup label="Parent Categories (Top-Level)">
                      {categories
                        .filter((c) => !c.parentCategory)
                        .map((c) => (
                          <option key={c._id} value={c._id}>
                            {c.name} [Parent Category]
                          </option>
                        ))}
                    </optgroup>
                    <optgroup label="Other / Subcategories">
                      {categories
                        ...
                        .map((c) => (
                          <option key={c._id} value={c._id}>
                            {c.name} (Sub under {c.parentCategory?.name || 'Parent'})
                          </option>
                        ))}
                    </optgroup>
                  </select>
